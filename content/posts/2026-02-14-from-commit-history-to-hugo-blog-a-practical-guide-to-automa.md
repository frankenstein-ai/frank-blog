...
1. **Separate notebooks / memos repo** -- Many of our research labs store notebooks in a *lab-work* repository while the source code lives elsewhere. The original CLI assumed the same repository contained both the input (commits) and the output (blog posts).

2. **Multiple content pipelines** -- Some teams run a *notebook* workflow that turns commits into research notes, and a *blog* workflow that reads those notes and turns them into public posts. Tying the two together in a single configuration was awkward.

3. **Hard-to-reproduce GitHub Actions** -- We wanted to drop a ready-made GitHub Actions file into any repo and have it work out of the box. The existing `generate.yaml` was too generic and required a lot of boilerplate.
...
## The solution: a new configuration layer and reusable workflows

The recent commits introduced three major changes:

1. **`blog_source_repo` configuration** -- A dedicated field that overrides the regular `source_repo` for *blog post* generation. It allows the CLI to read notebooks and memos from a different repository than the one it updates with commits.

2. **Workflow templates in `examples/workflow/`** -- Two ready-to-drop GitHub Actions files: one for generating notebooks/memos, another for turning those into blog posts, updating the Hugo menu, and regenerating the home page.

3. **Documentation overhaul** -- Both the README and the CLAUDE.md were updated to expose the new flags, environment variables, and configuration files. The docs now walk a user through setting up the two workflows side by side.
...
The main repo can focus on code while a dedicated notes repo holds notebooks and memos. The same `frank generate blog-posts` command now works in a repo that only contains source code, without touching the notes repo. Each job in GitHub Actions can check out the appropriate repo, run the relevant command, and push results to the right place.
...
The templates are fully parameterised via environment variables and repository variables.
...
The README now contains a dedicated workflow templates section that walks through both files, explains the environment variables, and shows a typical setup flow. The CLAUDE.md is updated to reflect the new environment variable names and the new `workflow/` directory in the examples tree.

## 3. Documentation overhaul

### README highlights

The README now enumerates both `generate-notes.yaml` and `generate-blog.yaml` with a clear copy-paste-and-go path. A table shows the config precedence order: CLI flags > env vars > `.frank.toml` > defaults. The `--blog-source-repo` flag is listed under command-specific flags, clarifying its scope. And the example `.frank.toml` now includes `blog_source_repo = ""` so that users can see the placeholder.

### CLAUDE.md updates

The CLAUDE.md now lists the new environment variable `FRANK_BLOG_SOURCE_REPO` and the new workflow directory `workflow/`. This aligns the contribution guidance with the new feature set.
...
1. **Lab-Work Repo** (`frankenstein-ai/lab-work`) -- Holds notebooks and memos in `notebooks/` and `insight_memos/`.

2. **Blog Repo** (`frankenstein-ai/frank-blog`) -- Hugo site; `content/posts/` is where generated markdown lands.

3. **Source Repo** (`frankenstein-ai/my-rd-project`) -- Holds the actual code; commits trigger the notebooks workflow.
...
> **Tip:** If your repository has a large history, consider running `frank init --commit <oldest>` only once, then rely on the state DB to keep the runtime reasonable. The `--dry-run` flag is useful for estimating token usage before committing to a paid LLM plan.

## Why this matters for other projects

Committing code and getting both internal memos and public blog posts from the same diff eliminates duplicate effort. By separating the source for notebooks and the source for blog posts, you keep your main repo lean while still producing rich content from a dedicated content repo.

The ready-made GitHub Actions files mean you can spin up a content pipeline in minutes without wrestling with API keys or custom scripts. The tool itself is pure Go with no CGo dependencies, using raw HTTP calls, which makes it easy to audit or extend. And the prompts are embedded via `go:embed`, so you can tweak the LLM prompts without touching the code.
